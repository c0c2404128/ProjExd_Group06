...
### 分担追加機能
* **自動スクロール背景* **: 戦闘中、背景が自動でスクロールし続ける。 (担当: 佐藤)
* **敵キャラクター**:
    * 専用の画像を持つ敵が出現。 (担当: 佐藤)
    * 敵は体力（HP）を持ち、体力バーで可視化される。 (担当: 浅井)
    * ゲームの進行度（時間）に応じて、敵の最大HPが増加していく。 (担当: 浅井)
* **選択ゲート（パワーアップ）**:
    * 「連射速度UP」「矢の数+1」などのバフ効果が得らる。 (担当: 金澤)
    * 「連射速度DOWN」などのデバフ効果も存在する。 (担当: 金澤)
* **ボス戦**:
    * ゲート通過2回ごとにボスが出現する。 (担当: 岡倉)
    * ボスは左右に移動しながら、プレイヤーを狙って爆弾を投下する攻撃を行う。 (担当: 岡倉)
* **スコアシステム**:
    * 敵を倒すとスコアが加算される（ザコ敵: 10点, ボス: 100点）。 (担当: 竹前)
    * ボス撃破時には「ボス撃破！」というメッセージが画面中央に表示される。 (担当: 竹前)
